...
class RuleEvaluationService {
   ...
   */
  async evaluateRules(category, planets, doshas, yogas, dasha, nakshatraName) {
   ...
   */
  evaluateRuleConditions(conditions, planets, doshas, yogas, dasha, nakshatraName) {
    ...
    if (conditions.mahadasha && conditions.mahadasha.length > 0 && dasha) {
   ...
   */
  evaluatePlanetCondition(condition, planets) {
    const planet = planets.find(
      p => p.name && p.name.toLowerCase() === condition.name.toLowerCase()
    );
    ...
    if (condition.dignity && condition.dignity.length > 0) {
      const planetDignity = this.getPlanetDignity(planet);
      matches = matches && condition.dignity.includes(planetDignity);
    }
   ...
   */
  evaluateDoshaCondition(condition, doshas) {
    const dosha = doshas.find(
      d => d.name && d.name.toLowerCase() === condition.name.toLowerCase()
    );
   ...
   */
  evaluateYogaCondition(condition, yogas) {
    const yoga = yogas.find(
      y => y.name && y.name.toLowerCase() === condition.name.toLowerCase()
    );
   ...
   */
  evaluateHouseCondition(houses, planets) {
