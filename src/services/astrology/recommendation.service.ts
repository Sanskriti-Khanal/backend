...
import { RudrakshaCategoryModel } from '../../models/RudrakshaCategory.model';
...
export class RecommendationService {
  ...
  private async getRuleBasedRecommendations(
    ...
    for (const rule of matchedRules) {
      const ruleRecommendations = rule.recommendations as RuleRecommendation;

      // Process Rudraksha recommendations
      if (ruleRecommendations.rudraksha) {
        for (const rudrakshaRec of ruleRecommendations.rudraksha) {
          const rudrakshaCategory = await RudrakshaCategoryModel.findOne({
            mukhiCount: rudrakshaRec.mukhiCount,
            isActive: true,
          });
          ...
          if (rudrakshaCategory) {
            recommendations.push({
              ...
              benefits: rudrakshaRec.benefits || rudrakshaCategory.benefits || [],
              category,
      ...
      if (ruleRecommendations.gemstones) {
        for (const gemstoneRec of ruleRecommendations.gemstones) {
          const gemstoneCategory = await GemCategoryModel.findOne({
            name: { $regex: new RegExp(gemstoneRec.name, 'i') },
            isActive: true,
          });
          ...
          if (gemstoneCategory) {
            recommendations.push({
              ...
              benefits: gemstoneRec.benefits || gemstoneCategory.benefits || [],
              category,
    ...
    // Sort by priority (rules are already sorted, but we can re-sort if needed)
   ...
   */
  private getHardcodedRecommendations(
    planets: PlanetPosition[],
    doshas: Dosha[],
    yogas: Yoga[],
    category: RecommendationCategory
   ...
   */
  private analyzeCategoryIssues(
    planets: PlanetPosition[],
    doshas: Dosha[],
    yogas: Yoga[],
    category: RecommendationCategory
  ): Array<{ type: string; planet?: string; doshaName?: string }> {
    const issues: Array<{ type: string; planet?: string; doshaName?: string }> = [];
   ...
   */
  private getPlanetStrengtheningRecommendations(
    planetName: string,
    category: RecommendationCategory
  ): Recommendation[] {
    const recommendations: Recommendation[] = [];

    const planetGemstones: Record<string, { gemstone: string; rudraksha: string }> = {
   ...
   */
  private getPlanetRemedyRecommendations(
    planetName: string,
    category: RecommendationCategory
  ): Recommendation[] {
    const recommendations: Recommendation[] = [];
   ...
   */
  private getDoshaRemedyRecommendations(
    doshaName: string,
    category: RecommendationCategory
  ): Recommendation[] {
    const recommendations: Recommendation[] = [];
  ...
  private getCategorySpecificRecommendations(
    ...
    switch (category) {
      case 'wealth':
        // Jupiter and Venus for wealth
        const jupiter = planets.find((p) => p.name === 'Jupiter');
        const venus = planets.find((p) => p.name === 'Venus');
        if (jupiter && (jupiter.dignity === 'debilitated' || jupiter.house === 6 || jupiter.house === 8)) {
          recommendations.push({
            type: 'gemstone',
        ...
        if (venus && (venus.dignity === 'debilitated' || venus.house === 6 || venus.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Diamond',
      ...
      case 'health':
        // Sun and Moon for health
        const sun = planets.find((p) => p.name === 'Sun');
        const moon = planets.find((p) => p.name === 'Moon');
        if (sun && (sun.dignity === 'debilitated' || sun.house === 6 || sun.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Ruby',
            reason: 'Sun rules vitality and health',
            benefits: ['Improved vitality', 'Better health', 'Energy'],
            category: 'health',
            planet: 'Sun',
          });
        }
        if (moon && (moon.dignity === 'debilitated' || moon.house === 6 || moon.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Pearl',
            reason: 'Moon rules mind and emotional health',
            benefits: ['Mental peace', 'Emotional stability', 'Better sleep'],
            category: 'health',
            planet: 'Moon',
          });
        }
      ...
      case 'relationship':
        // Venus and Moon for relationships
        const venusRel = planets.find((p) => p.name === 'Venus');
        const moonRel = planets.find((p) => p.name === 'Moon');
        if (venusRel && (venusRel.dignity === 'debilitated' || venusRel.house === 6 || venusRel.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Diamond',
            reason: 'Venus rules love and relationships',
            benefits: ['Better relationships', 'Love and harmony', 'Marriage'],
            category: 'relationship',
            planet: 'Venus',
          });
        }
        if (moonRel && (moonRel.dignity === 'debilitated' || moonRel.house === 6 || moonRel.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Pearl',
            reason: 'Moon rules emotions and relationships',
            benefits: ['Emotional harmony', 'Better understanding', 'Peace'],
            category: 'relationship',
            planet: 'Moon',
          });
        }
      ...
      case 'career':
        // Sun and Mercury for career
        const sunCareer = planets.find((p) => p.name === 'Sun');
        const mercury = planets.find((p) => p.name === 'Mercury');
        if (sunCareer && (sunCareer.dignity === 'debilitated' || sunCareer.house === 6 || sunCareer.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Ruby',
        ...
        if (mercury && (mercury.dignity === 'debilitated' || mercury.house === 6 || mercury.house === 8)) {
          recommendations.push({
            type: 'gemstone',
            name: 'Emerald',
      ...
      case 'spiritual':
        // Jupiter and Ketu for spirituality
        const jupiterSp = planets.find((p) => p.name === 'Jupiter');
        const ketu = planets.find((p) => p.name === 'Ketu');
        if (jupiterSp && (jupiterSp.dignity === 'debilitated' || jupiterSp.house === 6 || jupiterSp.house === 8)) {
          recommendations.push({
            type: 'rudraksha',
   ...
   */
  private getPlanetBenefits(planetName: string, category: RecommendationCategory): string[] {
    const benefits: Record<string, Record<RecommendationCategory, string[]>> = {
      Sun: {
        ...
        study: ['Focus', 'Discipline', 'Recognition'],
      },
      Moon: {
        ...
        study: ['Memory', 'Calm focus', 'Comprehension'],
      },
   ...
   */
  private getRudrakshaBenefits(rudraksha: string, category: RecommendationCategory): string[] {
    const rudrakshaMeanings: Record<string, string[]> = {
   ...
   */
  private getFingerForPlanet(planetName: string): string {
   ...
   * Get day for planet
   */
  private getDayForPlanet(planetName: string): string {
   ...
   */
  private getPlanetRemedyInstructions(planetName: string): string {
